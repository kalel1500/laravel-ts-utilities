...
export class Ttable
{
    ...
    {
        const cells = row.getCells().filter((cell => { return cell.getColumn().getDefinition().editable !== false }));
        cells.forEach(cell => {
    ...
    static async defaultActionDelete(cell: CellComponent, url: string, {title = 'Eliminar', html = undefined, preventReplaceData = false}: {title?: string, html?: string, preventReplaceData?: boolean}): Promise<boolean>
    {
        try {
